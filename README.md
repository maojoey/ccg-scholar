...
- **30 个命令模板**: 在 Claude Code 中通过 `/ccg-scholar:xxx` 即开即用
...
| `/ccg-scholar:team-plan` | 协作研究计划 | Codex 技术架构 + Gemini 方法论 + Claude 策略 → 统一 DAG |
| `/ccg-scholar:team-exec` | 按 DAG 并行执行 | 拓扑排序分 Wave，多模型并行调度 |
...
计划与执行
  /ccg-scholar:team-plan                  → 多模型并行制定计划 → 任务 DAG
  /ccg-scholar:team-exec                  → 按 Wave 并行执行任务 DAG
...
- **30 Command Templates**: Use via `/ccg-scholar:xxx` in Claude Code
...
| **Team** (5) | `team-research`, `team-plan`, `team-exec`, `team-write`, `team-review` |
...
/ccg-scholar:team-plan           → Multi-model parallel planning → task DAG
/ccg-scholar:team-exec           → Execute task DAG in parallel waves
